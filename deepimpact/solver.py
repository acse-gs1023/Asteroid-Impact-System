...
class Planet:
    ...
    def solve_atmospheric_entry(
        ...
        results = []
        fragmented = False
        # Initialize the user-specified time elapsed counter
        ...
        while True:
            ...
            if user_time_elapsed >= dt:
                ...
                results.append([t] + list(y0))
                user_time_elapsed = 0.0
                # Reset the user-specified time elapsed counter
    ...
    def calculate_energy(self, result):
        ...
        # prepend the first value to maintain array size
        ...
        altitude_diff = np.diff(
            result["altitude"], prepend=result["altitude"][0])

        # Replace any zero altitude differences,
        # with a small value to avoid division by zero
    ...
    def analyse_outcome(self, result):
        ...
        if max_dedz_altitude > 0:
            ...
            # Calculate the kinetic energy loss,
            # from initial altitude to burst altitude
            ...
            # For cratering, determine the specifics,
            # at the point of ground impact
            impact_index = result[result["altitude"] <= 0].index[
                0
            ]  # Index of ground impact
            initial_kinetic_energy = (
                0.5 * result.loc[0, "mass"] * result.loc[0, "velocity"] ** 2
            )
