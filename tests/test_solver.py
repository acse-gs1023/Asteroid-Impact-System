...
def test_energy_calculation_varying_mass(planet):
 result = pd.DataFrame({'mass': [1000, 500], 'velocity': [100, 100], 'altitude': [10000, 9500]})
...
def test_energy_calculation_constant_mass(planet):
 result = pd.DataFrame({'mass': [1000, 1000], 'velocity': [100, 200], 'altitude': [10000, 9500]})
 updated_result = planet.calculate_energy(result)
 assert updated_result['dedz'].iloc[0] != updated_result['dedz'].iloc[1]
 
def test_outcome_airburst_high_altitude(planet):
 result = pd.DataFrame({
 'dedz': [0, 5, 10, 5, 0],
 'altitude': [10, 8, 6, 4, 2],
 'distance': [0, 1, 2, 3, 4]
...
def test_outcome_airburst_energy_calculation(planet):
 result = pd.DataFrame({
 'dedz': [0, 5, 10, 5, 0],
 'altitude': [10, 8, 6, 4, 2],
 'distance': [0, 1, 2, 3, 4]
 ...
 outcome = planet.analyse_outcome(result)
 assert outcome['burst_energy'] == 10
