...
def test_solve_atmospheric_entry(result):
    ...
    print("now you are in the function1 test")
    print
...
def test_scenario(planet):
    ...
    print(111111)
    print(result)
    expected_data = np.load("scenario.npz")
    df = pd.DataFrame()
    for key in expected_data.files:
        df[key] = expected_data[key]
    print(df)

    # # 比较每一列的数据
    # for key in expected_keys:
    #     expected_column = expected_data[key]
    #     result_column = result[key].to_numpy()
    #     print(11111)
    #     assert np.allclose(result_column, expected_column, atol=1)
...
if __name__ == "__main__":
    ...
    # 执行函数
    print(planet)
    result1 = planet.solve_atmospheric_entry(**inputs)
    print(result1)
